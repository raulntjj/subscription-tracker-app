...
export function SubscriptionDialog({ open, onOpenChange, initialData, trigger }: Props) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {initialData ? t('editSubscriptionDesc') : t('createSubscriptionDesc')}
          </DialogDescription>
        </DialogHeader>

        <SubscriptionForm initialData={initialData} onClose={() => onOpenChange(false)} />
      </DialogContent>
    </Dialog>
  );
}
