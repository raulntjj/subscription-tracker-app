...
type SubscriptionsPaginatedData = PaginatedData<{ subscriptions: Subscription[] }>;

export async function getSubscriptions(
  params?: PaginationParams
): Promise<ApiResponse<SubscriptionsPaginatedData>> {
  const { data } = await apiClient.get<ApiResponse<SubscriptionsPaginatedData>>(BASE_URL, {
    params,
  });
  return data;
}
